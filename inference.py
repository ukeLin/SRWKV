...
def test_acdc(ckpt: str) -> None:
    ...
    dataset = ACDCDataset(base_dir="/root/autodl-tmp/dataset/ACDC", split="test")
...
def test_synapse(ckpt: str) -> None:
    ...
    dataset = SynapseDataset(base_dir="/root/autodl-tmp/dataset/Synapse/test_vol_h5", split="test_vol")
    dataloader = DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0)
    inference(
        model=model,
        dataloader=dataloader,
