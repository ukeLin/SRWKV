...
#define CHANNEL_SPLIT (512/32)  // Set to 4 for balanced performance
#define EPS (1e-6)
#define TOKEN_SPLIT (512 / CHANNEL_SPLIT) // the number of split tokens = 128  
...
__global__ void kernel_forward(const int B, const int T, const int C,
    ...
    // Boundary check: return if thread is beyond valid range
    if (idx >= B * C) return;
...
void cuda_forward(int B, int T, int C, float *w, float *u, float *k, float *v, float *y) {
    ...
    // Use ceiling division to handle cases where B*C is not perfectly divisible
