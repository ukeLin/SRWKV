...
wkv6_cuda = load(name="wkv6",
                 sources=["/root/autodl-tmp/SRWKV/model/mmcls_custom/models/backbones/cuda_v6/wkv6_op.cpp", 
                          "/root/autodl-tmp/SRWKV/model/mmcls_custom/models/backbones/cuda_v6/wkv6_cuda.cu"],
...
@BACKBONES.register_module()
class VRWKV6_Hierarchical(BaseBackbone):
    def __init__(self,
                 img_size=64,
                 in_channels=3,
                 embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8],  # HEAD_SIZE=64, 所以 embed_dims = num_heads * 64
                 depths=[2, 2, 2, 2],
                 drop_rate=0.,
                 drop_path_rate=0.1,
                 shift_pixel=1,
        ...
        # 验证embed_dims和num_heads的关系
        for i, (emb_dim, n_head) in enumerate(zip(embed_dims, num_heads)):
            assert emb_dim == n_head * HEAD_SIZE, \
                f"Stage {i}: embed_dims ({emb_dim}) must equal num_heads ({n_head}) * HEAD_SIZE ({HEAD_SIZE})"
        ...
        for i in range(self.num_stages):
            # Patch embedding/Downsampling layer
            if i == 0:
                # 第一个stage: 3 channels -> embed_dims[0], img_size -> img_size//4
                patch_embed = nn.Sequential(
                    nn.Conv2d(in_channels, embed_dims[i], kernel_size=4, stride=4),
                    nn.LayerNorm([embed_dims[i], img_size//4, img_size//4])
                )
            else:
                # 后续stage: 2x下采样
                patch_embed = nn.Sequential(
                    nn.Conv2d(embed_dims[i-1], embed_dims[i], kernel_size=2, stride=2),
                    nn.LayerNorm([embed_dims[i], (img_size//(4*(2**i))), (img_size//(4*(2**i)))])
                )
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            seq_len = H * W
            if seq_len > T_MAX:
                logger.warning(f"Stage {i}: sequence length {seq_len} exceeds T_MAX={T_MAX}. "
