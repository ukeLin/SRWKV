...
torch::Tensor bi_wkv_cuda_forward(
    ...
    // Use ceiling division to handle cases where batch_size*num_channels is not perfectly divisible
    const dim3 blocks((batch_size * num_channels + threads.y - 1) / threads.y);
